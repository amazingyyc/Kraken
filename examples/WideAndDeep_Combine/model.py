...
class WideDeep(nn.Module):
  ...
  def __init__(self, wide_dim, embeddings_input, continuous_cols, deep_column_idx, hidden_layers, dropout,
    ...
    dimensions = []
    names = []
    for name, _, dim in self.embeddings_input:
      dimensions.append(dim)
      names.append(name)

    setattr(self, 'emb_layers', kk.CombineEmbedding(dimensions=dimensions, names=names))
  ...
  def forward(self, X_w, X_d):
    """Implementation of the forward pass.
        ...
        """
    # Deep Side
    cols = [X_d[:, self.deep_column_idx[col]].long() for col, _, _ in self.embeddings_input]
    emb = getattr(self, 'emb_layers')(cols)
    # emb = [
